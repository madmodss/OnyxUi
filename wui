...
RunService.Heartbeat:Connect(function(dt)
	for instance, state in next, springStates do
		for propName, spring in next, state do
...
function spr.target(instance, dampingRatio, frequency, properties)
	...
	for propName, propTarget in next, properties do
